...
export default async function DashboardPage() {
  ...
  const omzet = (finance || []).filter((f) => f.type === "pemasukan").reduce((a, b) => a + Number(b.amount), 0);
  const pengeluaran = (finance || []).filter((f) => f.type === "pengeluaran").reduce((a, b) => a + Number(b.amount), 0);

  const STATS: { label: string; value: string; delta: string; trend: "up" | "down"; sub: string }[] = [
    { label: "Total servis", value: String(totalServis ?? 0), delta: `${pending?.length ?? 0} pending`, trend: "up", sub: "vs. kemarin" },
    { label: "Omzet hari ini", value: `Rp ${(omzet / 1000).toFixed(0)}k`, delta: `+${omzet ? "8.2%" : "0%"}`, trend: "up", sub: `Keluar Rp ${(pengeluaran / 1000).toFixed(0)}k` },
    { label: "Garansi aktif", value: String(garansiAktif ?? 0), delta: "3 bln", trend: "up", sub: "auto 90 hari" },
    {
      label: "Menunggu konfirmasi",
      value: String(pending?.length ?? 0),
      delta: pending && pending.length > 0 ? "-follow up" : "aman",
      trend: pending && pending.length > 0 ? "down" : "up",
      sub: "WA Fonnte",
    },
  ];

  const SPARK = [2, 3, 5, 4, 6, 5, 8, 7, 9, 8, 10, totalServis ?? 8];
  const ACTIVITY =
    (recentServis || []).map((s) => ({
    ...
    <div className="min-h-svh bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-10 py-8">
        ...
        <div className="mt-6 grid grid-cols-1 gap-3 lg:grid-cols-3">
          <ChartCard data={SPARK} total={String(totalServis ?? 0)} />
...
function ChartCard({ data, total }: { data: number[]; total: string }) {
  return (
    <div className="rounded-xl border border-border/60 bg-background/40 p-5 lg:col-span-2">
      <div className="flex items-end justify-between">
        ...
        <div className="flex items-center gap-1 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em]">
          <button type="button" className="rounded px-2 py-1 transition-colors hover:bg-foreground/[0.05]">7d</button>
          <button type="button" className="rounded bg-foreground/[0.08] px-2 py-1 text-foreground">30d</button>
          <button type="button" className="rounded px-2 py-1 transition-colors hover:bg-foreground/[0.05]">90d</button>
        </div>
      </div>
