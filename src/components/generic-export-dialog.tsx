...
import { CalendarIcon, DownloadIcon, FileJsonIcon, FileSpreadsheetIcon, FileTextIcon } from "lucide-react";
...
const FORMATS: { id: Format; label: string; meta: string; Icon: typeof FileTextIcon }[] = [
  { id: "csv", label: "CSV", meta: "Spreadsheet", Icon: FileSpreadsheetIcon },
  { id: "xlsx", label: "Excel", meta: "XLSX", Icon: FileSpreadsheetIcon },
  { id: "json", label: "JSON", meta: "Raw", Icon: FileJsonIcon },
  { id: "pdf", label: "PDF", meta: "Cetak", Icon: FileTextIcon },
];

export type ExportField = { id: string; label: string; default: boolean };

export function GenericExportDialog({
  open,
  onOpenChange,
  title = "Export data",
  description = "Unduh data dalam format pilihan.",
  fields,
  rows,
  ...
  const handleExport = () => {
    const activeFields = fields.filter((f) => include[f.id]);
    const headers = activeFields.map((f) => f.label);
    const data = filteredRows.map((r) => {
      const row: Record<string, string> = {};
      for (const f of activeFields) {
        row[f.label] = String(r[f.id] ?? "");
      }
      return row;
    });

    const baseName = `${fileNamePrefix}-${format(new Date(), "yyyy-MM-dd")}`;
    ...
    if (fileFormat === "csv") {
      ...
      w.document.write(`
        ...
        <style>
          ...
          <h1>${title}</h1>
          <div class="meta">${format(new Date(), "d MMM yyyy", { locale: localeId })} · ${data.length} baris</div>
  ...
  const estimatedSize = useMemo(() => {
    const base = filteredRows.length * 120;
            ...
            <Popover>
              ...
              >
                ...
                <span className="font-mono text-[10px] text-muted-foreground">
                  {dateRange.from || dateRange.to ? `${filteredRows.length} baris` : "Semua"}
                </span>
              </PopoverTrigger>
